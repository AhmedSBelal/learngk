            ...
            <form method="POST" action="{{ route("admin.reviews.update", [$review->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                           id="name" value="{{ old('name', $review->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email"
                           id="email" value="{{ old('email', $review->email) }}" required>
                    @if($errors->has('email'))
                        <div class="invalid-feedback">
                            {{ $errors->first('email') }}
                        </div>
                ...
                <div class="form-group">
                    <label class="required" for="phone">{{ trans('cruds.review.fields.phone') }}</label>
                    <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone"
                           id="phone" value="{{ old('phone', $review->phone) }}" required>
                    @if($errors->has('phone'))
                        <div class="invalid-feedback">
                            {{ $errors->first('phone') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.review.fields.phone_helper') }}</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('review') ? 'is-invalid' : '' }}" name="review"
                              id="review" required>{{ old('review', $review->review) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                        ...
                        @foreach(App\Models\Review::STATUS_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('status', $review->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id"
                        ...
                        @foreach($users as $id => $entry)
                            <option
                                value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $review->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('user'))
                        <div class="invalid-feedback">
