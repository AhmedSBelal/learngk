            ...
            <form method="POST" action="{{ route("admin.jobs.update", [$job->id]) }}" enctype="multipart/form-data">
                ...
                @foreach(siteLanguages() as $locale)
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has($locale.'.name') ? 'is-invalid' : '' }}" type="text"
                               ...
                               value="{{ old($locale.'.name', $job->translate($locale, true)->name) }}"
                               required>
                    ...
                    <div class="form-group">
                        ...
                        <textarea
                            ...
                            id="description-{{$locale}}">{!! old($locale.'.description', $job->translate($locale, true)->description) !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug"
                           id="slug" value="{{ old('slug', $job->slug) }}" required>
                    @if($errors->has('slug'))
                        <div class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </div>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="active" id="active"
                               value="1" {{ $job->active || old('active', 0) === 1 ? 'checked' : '' }}>
