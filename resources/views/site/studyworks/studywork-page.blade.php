...
@section('title', 'LGK - Study & Work in Germany')
@section('og_title', 'LGK - Study & Work in Germany')
@section('seo_description', settingText('study_work_seo_description', 'short_description'))
@section('og_description', settingText('study_work_seo_description', 'short_description'))
@section('seo_keywords', settingText('study_work_seo_keywords', 'short_description'))
@section('content')

 <header class="second-header py-5">
        <h1 class="second-header-title text-white text-center m-0">{{ request()->route()->parameter('type') == 'study' ? trans('website.header.study') : trans('website.header.work') }}</h1>
    </header>
    <section>
        <div class="container-fluid px-md-5">
            <form action="{{ route('study-work', $type) }}" method="GET" class="d-flex gap-4 flex-wrap my-5">
                @csrf
                <div class="position-relative">
                    <input type="text" name="search" class="form-control search-input-2 py-2 rounded-3" placeholder="بحث عن المقال" value="{{ request()->input('search') }}">
                ...
                <div class="d-none d-lg-flex rounded-3 border bg-transparent">
                    ...
                    <div class="py-2 px-4 d-flex gap-1 cards-filter long-cards hovers rounded-start-3 {{ $setStudyWorkView == 'grid' ? 'active' : '' }}" data-view="grid">
                        <div class="material-icons cards-filter-icon">grid_view</div>
                        <p class="m-0 cards-filter-label">{{ trans('website.course.grid_view') }}</p>
                    </div>
                </div>
                <button type="submit" class="primary-btn text-nowrap px-5 py-2 rounded-3 text-decoration-none">بحث</button>
            </form>
        </div>
    </section>
    ...
    <section>
        <div class="container-fluid mt-4 px-md-5">
            <div class="row mt-2 cards-container {{ $setStudyWorkView == 'list' ? 'list-view' : 'grid-view' }}">
                            ...
                            <div class="cards-long">
                                ...
                                <div class="card-body px-0">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mt-3">
                                        <a href="{{ route('study-work.detail', $article->slug) }}" class="read-more">{{ trans('website.global.read') }}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const savedView = localStorage.getItem('setStudyWorkView') || '{{ $setStudyWorkView }}';
            ...
            viewButtons.forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    localStorage.setItem('setStudyWorkView', view);
                    updateView(view);

                    fetch('{{ route('set-study-work-view') }}', {
                        method: 'POST',
                        headers: {
